...
├── LICENSE                   # MIT License
├── fixed_pipeline.py         # Main pipeline — reproduces all results reported in the paper
├── bayesian.py               # Hyperparameter search — exploratory, not required to reproduce results
└── data/                     # Place datasets here (see Data section)
```

### Which file should I run?

| File | Purpose | Run to reproduce paper? |
|---|---|---|
| `fixed_pipeline.py` | Full pipeline with pre-set optimal hyperparameters | **Yes** |
| `bayesian.py` | Bayesian search that found those hyperparameters | No — exploratory only |

**`fixed_pipeline.py`** is the authoritative script. Hyperparameters are hard-coded to the values
found via Bayesian optimization, so every run produces identical results (`random_state=42`).

**`bayesian.py`** documents how those values were found. Re-running it may yield slightly different
parameters due to the stochastic nature of Bayesian search. This is expected and does not affect
...
Run the full pipeline to reproduce all paper results:

```bash
python fixed_pipeline.py
```

The script trains all base classifiers and ensembles on both cohorts, performs cross-validation
and statistical testing, runs the data-leakage ablation, and generates the SHAP and LIME
interpretability outputs. Result tables and figures are written to the `sonuc/` directory
created at runtime.

To inspect the hyperparameter search process (optional):

```bash
python bayesian.py
```

> This is for reference only. Results may differ from those in the paper.
...
The data-leakage ablation showed that, under pre-split oversampling with regularization removed,
standalone models inflate sharply (KNN +27.00% to 0.9281; Random Forest +18.11% to 0.9162 on ILPD),
while the Blending ensemble remained the most resistant to this inflation.

## Citation

If you use this code, please cite the associated paper. The full citation will be added here once
the manuscript is published. In the meantime, you can reference this repository directly.
